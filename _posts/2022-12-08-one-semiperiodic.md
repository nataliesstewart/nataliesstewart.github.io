...
> **Definition 2.** *The space of* ordered n-periodic linear weaves *is be the subspace $$\cW^\sym_{\operatorname{per},n} \subset \cW^\sym_\omega$$ possessing a translational symmetry whose associated action on $$\omega$$ has $$n$$ orbits.* 
...
As before, we use the notation $$W_\lin := \pi_0 \cW_\lin$$, and similarly for other spaces.
...
> **Definition 7.** *A linear weave is* $$(r,n)$$-semiperiodic *if it posesses a symmetry composed of a translation and an order $$r$$ isometry of the plane perpendicular to the translation, such that the associated action on $$\omega$$ has $$n$$ orbits.*
>
...
There are potentially multiple $$(r,n)$$-semiperiodicity symmetries of a linear weave;
there is a well-defined $$(r,n)$$-semiunit, but there may exist multiple isometries of the plane perpendicular to the periodicity preserving the weave, and an $$(r,n)$$-semiperiodicity for each such isometry.

By discarding the information of *which* semiperiodicity realizes an $$(r,n)$$-semiperiodic weave, there is an evident $$\bP$$-filtration of $$W_{\lin}$$ whose $$(r,n)$$ part is the the $$\leq (r,n)$$-semiperiodic weaves.
By Lemma 4, the semiperiodicity filtration in some sense refines the periodicity filtration;
it's not to hard to see that the semiperiodicity filtration is finite type iff the periodicity filtration is.
